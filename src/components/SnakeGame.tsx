...
export default function SnakeGame({ width, height, startGameCallback, endGameCallback }: SnakeGameProps) {
	...
	const [originalHighScore,] = useState<string>(JSON.parse(localStorage.getItem("highscore") || "0"));
	...
	const snakeEat = (snakeBody: Position[]) => {
		...
		const newFoodPosition: Position = generateFood(snakeBody, width, height);
	...
	const updateGame = () => {
		if (gameOver) return;
		let newSnakeHeadUp: Position;
		let newSnakeUp: Position[];
		let newSnakeHeadDown: Position;
		let newSnakeDown: Position[];
		let newSnakeHeadLeft: Position;
		let newSnakeLeft: Position[];
		let newSnakeHeadRight: Position;
		let newSnakeRight: Position[];
		...
		switch (direction) {
			...
			//LEFT
			case Directions.UP:
				newSnakeHeadUp = getNewSnakeHead(snakeState, direction);
				newSnakeUp = [newSnakeHeadUp].concat(snakeState);
			...
			case Directions.DOWN:
				newSnakeHeadDown = getNewSnakeHead(snakeState, direction);
				newSnakeDown = [newSnakeHeadDown].concat(snakeState);
			...
			case Directions.LEFT:
				newSnakeHeadLeft = getNewSnakeHead(snakeState, direction);
				newSnakeLeft = [newSnakeHeadLeft].concat(snakeState);
			...
			case Directions.RIGHT:
				newSnakeHeadRight = getNewSnakeHead(snakeState, direction);
				newSnakeRight = [newSnakeHeadRight].concat(snakeState);
